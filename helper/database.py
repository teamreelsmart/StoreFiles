...
class MongoDB:
    ...
    async def create_verify_link(self, token: str, user_id: int, payload: str, short_link: str, expires_at: datetime, public_token: str | None = None):
        await self.verify_links.update_one(
            ...
            {"$set": {
                ...
                "used": False,
                "public_token": public_token or token
            }},
            upsert=True
        )
    ...
    async def get_verify_link_by_public_token(self, public_token: str) -> dict:
        return await self.verify_links.find_one({"public_token": public_token})
