...
class MongoDB:
    ...
    async def create_verify_link(self, token: str, user_id: int, payload: str, short_link: str, expires_at: datetime, provider: str = "primary"):
    ...
    async def get_next_shortner_provider(self, user_id: int) -> str:
        data = await self.verify_users.find_one({"_id": user_id})
        if not data:
            return "primary"
        return "secondary" if data.get("last_shortner_provider") == "primary" else "primary"

    async def set_last_shortner_provider(self, user_id: int, provider: str):
        await self.verify_users.update_one(
            {"_id": user_id},
            {"$set": {"last_shortner_provider": provider, "updated_at": datetime.now()}},
            upsert=True
        )
