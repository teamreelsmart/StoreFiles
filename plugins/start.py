...
async def issue_verify_link(client: Client, message: Message, payload: str):
    ...
    public_token = secrets.token_urlsafe(8).replace('-', '').replace('_', '')[:10]
    ...
    await client.mongodb.create_verify_link(token, message.from_user.id, payload, short_link, expires_at, public_token=public_token)
    ...
    service_link = build_verify_path(client, public_token)
